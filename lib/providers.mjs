...
export const LADDER = [
  ...
  // Survival rung for ~0.6 Mbps (2G/weak-3G) connections: ~440 kbps total,
  // comfortably under a 600 kbps link. The managed platforms all ship one.
...
export async function encodeHlsLadder(input, outDir, { onLog = () => {}, onProgress = () => {} } = {}) {
  ...
  // Poster frame for the player (non-fatal if it fails).
  try {
    onLog("Generating poster.jpg…");
    await runFfmpeg(ffmpeg, [
      "-y", "-ss", "2", "-i", input,
      "-frames:v", "1", "-vf", "scale=-2:720", "-q:v", "3",
      path.join(outDir, "poster.jpg"),
    ], null, null);
  } catch (err) {
    onLog(`Poster generation failed (continuing): ${err.message}`);
  }
