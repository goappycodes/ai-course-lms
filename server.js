...
app.get("/api/config", (_req, res) => {
  ...
  // --- Cloudflare Stream ---
  const streamCode = cur.stream?.customerCode || env.CF_STREAM_CUSTOMER_CODE;
  const streamUid = cur.stream?.uid || env.CF_STREAM_VIDEO_UID;
  ...
  // --- MUX ---
  const muxPlaybackId = cur.mux?.playbackId || env.MUX_PLAYBACK_ID;
  ...
  // --- S3 + CloudFront ---
  const s3Key = cur.s3?.key || env.S3_VIDEO_KEY;
  const s3Configured = Boolean(env.CLOUDFRONT_URL && s3Key);
  const s3 = {
    configured: s3Configured,
    url: s3Configured ? joinUrl(env.CLOUDFRONT_URL, s3Key) : DEMO.mp4,
  };

  // --- R2 + self-managed HLS ---
  const r2Prefix = cur.r2?.prefix || env.R2_VIDEO_PREFIX;
  const r2Configured = Boolean(env.R2_PUBLIC_URL && r2Prefix);
  const r2 = {
    configured: r2Configured,
    url: r2Configured
      ? joinUrl(env.R2_PUBLIC_URL, `${r2Prefix.replace(/\/+$/, "")}/master.m3u8`)
      : DEMO.hls,
  };
  ...
  const upload = {
    ...
    s3: Boolean(env.AWS_ACCESS_KEY_ID && env.AWS_SECRET_ACCESS_KEY && env.S3_BUCKET),
    r2: Boolean(env.R2_ACCOUNT_ID && env.R2_ACCESS_KEY_ID && env.R2_SECRET_ACCESS_KEY && env.R2_BUCKET),
  };

  res.json({ stream, mux, s3, r2, youtube, upload, current: cur.slug || null });
});
